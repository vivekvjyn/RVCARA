...
bool ConversionModification::setSettings (const engine::ConversionSettings& newSettings)
{
...
bool ConversionModification::readFromArchive (juce::InputStream& stream)
{
    ...
    // Read into a local and commit only once the whole record is read, so a truncated
    // archive leaves the modification at its defaults rather than half-configured.
    const auto storedVoiceName = stream.readString();

    engine::ConversionSettings storedSettings;
...
void ConversionModification::readAndDiscard (juce::InputStream& stream)
{
    ...
    stream.readString();    // voice name
    stream.readFloat();     // pitch shift
    stream.readFloat();     // retrieval ratio
    stream.readFloat();     // consonant protection
    stream.readFloat();     // envelope follow
    stream.readInt();       // latent seed
    stream.readBool();      // bypass
}
