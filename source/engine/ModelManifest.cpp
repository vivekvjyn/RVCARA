...
namespace
{
    /** Fetches a required child object, recording an error if it is missing. */
    ...
    /** Reads an optional numeric field, falling back to a default. */
    template <typename Type>
    ...
    /** Reads a graph's file and tensor names out of its manifest entry.

        Tensor names are taken from the graph description rather than assumed, which is
        what lets a third-party RVC export with different names be described by editing
        the manifest instead of the source.
    */
...
std::optional<ModelManifest> ModelManifest::parse (const juce::File& file, juce::String& error)
{
    ...
    // Retrieval is optional: a model exported with --skip-retrieval, or trained without
    // an index, is still usable with retrieval disabled.
    ...
    if (const auto filterValue = root->getProperty ("highPassFilter"); auto* filter = filterValue.getDynamicObject())
    {
        ...
        if (const auto* sections = filter->getProperty ("sections").getArray())
        {
            for (const auto& sectionValue : *sections)
            {
                ...
                dsp::BiquadCoefficients section;
    ...
    // Cross-checks that catch a manifest which parsed but describes something
    // impossible, rather than letting it fail obscurely mid-render.
