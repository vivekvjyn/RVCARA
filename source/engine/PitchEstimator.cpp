...
double PitchEstimator::decodeFrame (const float* salienceFrame) const
{
    ...
    // Weighted mean over the window around the peak. Classes outside the array are
    // skipped, which is equivalent to the reference's zero padding: a padded entry
    // contributes zero salience and a zero cents value, so it changes neither sum.
    ...
    // Zero cents means the weighted mean landed on the padded region, which the
    // reference also treats as unvoiced rather than as 10 Hz.
    if (cents == 0.0)
        return 0.0;

    return dsp::centsToHz (cents, manifest.centsReferenceHz);
}
...
void PitchEstimator::fillUnvoicedGaps (std::vector<float>& frequencies)
{
    ...
    if (firstVoiced < 0)
        return; // nothing voiced: leave it silent rather than inventing a melody

    // Hold the first and last voiced values out to the ends, matching numpy.interp's
    // clamping behaviour.
    ...
    // Linear interpolation across each interior gap.
                                                 ...
                                                 juce::String& error) const
{
    ...
    // The U-Net halves the time axis once per stage, so the frame count must be a
    // multiple of 2^stages. Pad with zeros and discard the extra frames afterwards.
    ...
    // Shape is [1, paddedNumFrames, numPitchBins].
    ...
    // Transposition is applied after estimation, so it shifts the note the singer
    // actually sang rather than biasing the estimator.
    if (pitchShiftSemitones != 0.0f)
    {
        const auto ratio = static_cast<float> (dsp::semitonesToRatio (pitchShiftSemitones));
