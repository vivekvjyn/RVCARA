...
ara::DocumentController* PluginProcessor::getConversionDocumentController() const
{
    ...
    return juce::ARADocumentControllerSpecialisation::getSpecialisedDocumentController<ara::DocumentController> (
        araDocumentController);
}

engine::ConversionSettings PluginProcessor::getSettingsFromParameters() const
{
...
void PluginProcessor::setParametersFromSettings (const engine::ConversionSettings& settings)
{
...
std::vector<ara::ConversionModification*> PluginProcessor::getEditableModifications() const
{
    ...
    std::vector<ara::ConversionModification*> modifications;
    ...
    if (auto* playbackRenderer = getPlaybackRenderer<juce::ARAPlaybackRenderer>())
    {
        ...
        {
            auto* modification = playbackRegion->getAudioModification<ara::ConversionModification>();
...
void PluginProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer&)
{
    ...
    // Not bound to ARA. There is nothing useful to compute: the model needs the whole
    // performance before it can emit a sample, so the input passes through untouched and
...
{
    return new rvcara::plugin::PluginProcessor();
}

const ARA::ARAFactory* JUCE_CALLTYPE createARAFactory()
{
    return juce::ARADocumentControllerSpecialisation::createARAFactory<rvcara::ara::DocumentController>();
}
