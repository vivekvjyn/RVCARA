...
namespace
{
    /** Reflects an index into `[0, numSamples)` without repeating the edge sample.

        This is NumPy's and torch's `reflect` padding: the sample before the first is
        the second, not the first again. Repeating the edge would introduce a step the
        transform reads as a transient.
    */
      ...
      melFilterBank (std::move (filterBank))
{
    ...
    // Periodic Hann: the denominator is windowSize, not windowSize - 1.
...
{
    // Reflection needs at least as much signal as it mirrors.
...
int MelSpectrogram::process (const float* samples, int numSamples, std::vector<float>& destination) const
{
    ...
    // JUCE's real-only transform works in place on a buffer of 2 * fftSize floats and
    // leaves interleaved complex pairs for bins 0 to fftSize / 2.
