...
{
    // Wait rather than detach: a running job holds a reference to this loader.
        ...
        juce::MessageManager::callAsync (
            [this, name, error, loaded, onCompleted = std::move (onCompleted)]() mutable
            {
                withModelLocked ([this, &name, &error, &loaded]
                {
                    ...
                    // A newer request may have superseded this load while it ran; in that
                    // case discard the result rather than installing a voice the user has
