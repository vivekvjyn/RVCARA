...
namespace
{
    ...
    /** Reads a little-endian uint32 from a byte offset. */
...
BinaryMatrix BinaryMatrix::load (const juce::File& file)
{
    ...
    // The 32-byte header keeps the payload aligned for float access on every platform
    // the plugin targets, so the data is used in place rather than copied out.
